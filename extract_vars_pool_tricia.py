...
def get_files(testset, startdate, ii, wrfdir):
    ...
    # add the previous file for accumulating variables 
    prevdatestr = (startdate + dt.timedelta(days=ii-1)).strftime("%Y-%m-%d")
    additional = Path(wrfdir) / f"wrfout_{testset}_{prevdatestr}_18:00:00"
    if not additional.exists():
        raise Exception(f"File {additional} is required to extract accumulating variables")
    filelist.insert(0,additional)
...
def process_and_save_files(testset, startdate, ii, args, COMPRESSIONLEVEL, SUBSETS, retrieve_vars, postprocess_acc_all):
...
def process_day(testset, startdate, ii, args, COMPRESSIONLEVEL, SUBSETS, retrieve_vars, postprocess_acc_all):
...
if __name__ == '__main__':
    ...
    with Pool() as pool:
        for testset in SUBSETS:
            args_list = [(testset, startdate, ii, args, COMPRESSIONLEVEL, SUBSETS, retrieve_vars, postprocess_acc_all) for ii in range(args.ndays)]
            pool.starmap(process_day, args_list)
